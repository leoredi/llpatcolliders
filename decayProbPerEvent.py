"""
Heavy Neutral Lepton (HNL) Decay Probability Analysis

This script analyzes HNL particles from PYTHIA simulations and calculates
decay probabilities within a detector tube geometry. HNLs are long-lived
particles that can travel significant distances before decaying.

Analysis Pipeline:
1. Read HNL particle data from CSV (event, eta, phi, momentum, mass)
2. Calculate decay lengths based on lifetime and boost factors
3. Determine path lengths through detector tube using ray-tracing
4. Compute decay probabilities for individual particles and events
5. Scan over lifetime parameter space to generate exclusion plots
"""
...
if __name__ == "__main__":
    # CSV file containing HNL particle data from PYTHIA simulation
    # This should be generated by running: ./main144 -c hnlLL.cmnd
    # Then converting ROOT output: python convertRootToCsv_HNL.py
    sample_csv = "LLP.csv"
    # create_sample_csv(sample_csv, n_events=500)  # Uncomment to generate test data

    # Set origin (interaction point)
    origin = [0, 0, 0]

    # Single lifetime analysis
    print("\n" + "="*60)
    print("HNL SINGLE LIFETIME ANALYSIS")
    print("="*60)

    lifetime = 1e-8  # 10 nanoseconds (adjustable)
    df_results = process_particle_csv(sample_csv, mesh, origin, lifetime)

    # Print summary statistics
    event_stats = df_results.groupby('event').first()

    print(f"\nResults for HNL lifetime = {lifetime*1e9:.1f} nanoseconds:")
    print(f"Total events: {len(event_stats)}")
    print(f"Total HNL particles: {len(df_results)}")

    particles_hitting = df_results[df_results['hits_tube'] == True]
    print(f"\nParticle-level statistics:")
    print(f"HNLs hitting detector tube: {len(particles_hitting)} ({len(particles_hitting)/len(df_results)*100:.1f}%)")
    if len(particles_hitting) > 0:
        print(f"Mean HNL decay probability (for hits): {particles_hitting['decay_probability'].mean():.4f}")

    print(f"\nEvent-level statistics:")
    print(f"Events with at least one HNL hitting tube: {(event_stats['n_particles_hitting_tube'] > 0).sum()}")
    print(f"Events with both HNLs hitting tube: {(event_stats['n_particles_hitting_tube'] == 2).sum()}")
    print(f"Mean P(at least one HNL decays): {event_stats['prob_at_least_one_decays'].mean():.6f}")
    print(f"Mean P(both HNLs decay): {event_stats['prob_both_decay'].mean():.6f}")
    print(f"Mean P(exactly one HNL decays): {event_stats['prob_exactly_one_decays'].mean():.6f}")

    # Save results
    df_results.to_csv("HNL_particle_decay_results.csv", index=False)
    event_stats.to_csv("HNL_event_decay_statistics.csv")
    print("\nDetailed results saved to CSV files")

    # Lifetime scan
    print("\n" + "="*60)
    print("HNL LIFETIME SCAN ANALYSIS")
    print("="*60)
    ...
    fig.suptitle('Heavy Neutral Lepton (HNL) Decay Analysis', fontsize=14, fontweight='bold')

    # Plot 1: Single particle decay probability
    ax1 = axes[0, 0]
    ax1.semilogx(lifetimes * 1e9, scan_results['mean_single_particle_decay_prob'],
                 'b-', linewidth=2, label='Single HNL (hits only)')
    ax1.set_xlabel('HNL Lifetime (nanoseconds)', fontsize=11)
    ax1.set_ylabel('Mean Decay Probability', fontsize=11)
    ax1.set_title('Single HNL Decay Probability vs Lifetime', fontsize=11)
    ...
    ax2.loglog(lifetimes * 1e9, scan_results['mean_at_least_one_decay_prob'],
               'r-', linewidth=2, label='At least one HNL decays')
    ax2.loglog(lifetimes * 1e9, scan_results['mean_both_decay_prob'],
               'g--', linewidth=2, label='Both HNLs decay')
    ax2.set_xlabel('HNL Lifetime (nanoseconds)', fontsize=11)
    ax2.set_ylabel('Event Decay Probability', fontsize=11)
    ax2.set_title('Event-Level HNL Decay Probabilities', fontsize=11)
    ...
    ax3.loglog(lifetimes * 1e9, scan_results['mean_at_least_one_decay_prob'],
               'r-', linewidth=2, label='P(≥1 HNL decay per event)')
    ax3.loglog(lifetimes * 1e9, scan_results['mean_single_particle_decay_prob'],
               'b--', linewidth=2, label='P(single HNL decay)')
    ax3.set_xlabel('HNL Lifetime (nanoseconds)', fontsize=11)
    ax3.set_ylabel('Decay Probability', fontsize=11)
    ax3.set_title('Event vs Single HNL Decay Probabilities', fontsize=11)
    ax3.grid(True, which="both", ls="-", alpha=0.2)
    ax3.legend()

    # Plot 4: Exclusion limits for HNL
    ax4 = axes[1, 1]
    ax4.loglog(lifetimes * 3E8, scan_results['exclusion'],
                 'm-', linewidth=2, label="HNL (this analysis)")
    ax4.set_xlabel('Decay Length (m)', fontsize=11)
    ax4.set_ylabel('Branching Ratio', fontsize=11)
    ax4.set_title('HNL Exclusion Limits', fontsize=11)
    ...
    plt.savefig('HNL_exclusion_vs_lifetime.png', dpi=150)
    plt.show()
