...
def load_br_vs_lifetime_data(mass_gev, scenario='mu'):
    ...
    Args:
        ...
        scenario: 'mu' or 'tau' (or 'e' for electron)

    Returns:
        (lifetimes_array, br_limits_array) or None if data not available
    """
    # Map scenario to lepton name
    lepton_map = {'mu': 'muon', 'muon': 'muon', 'tau': 'tau', 'e': 'electron', 'electron': 'electron'}
    lepton_name = lepton_map.get(scenario, 'muon')

    # Check for the exclusion data CSV file (new format: output/csv/HNL_mass_15.0_muon_exclusion_data.csv)
    exclusion_file = f"output/csv/HNL_mass_{mass_gev}_{lepton_name}_exclusion_data.csv"

    if not os.path.exists(exclusion_file):
        print(f"Warning: Exclusion data not found: {exclusion_file}")
        print(f"Run: python decayProbPerEvent.py csv/HNL_mass_{mass_gev}_{lepton_name}.csv")
        return None
    ...
    print(f"Loaded exclusion data for m={mass_gev} GeV ({lepton_name}): {len(lifetimes)} points")

    return lifetimes, br_limits

def compute_coupling_limit(mass_gev, br_limits, lifetimes, lepton='mu'):
    """
    Convert BR vs lifetime exclusion to a |U_ℓ|^2 limit at fixed mass.

    For each test value of |U_ℓ|^2 we compute:
      - BR_expected(|U|^2, m_N) from br_w_to_hnl
      - τ_expected(|U|^2, m_N) from hnl_lifetime

    The coupling is excluded if:
      BR_expected(|U|^2, m_N) > BR_limit(τ_expected)

    IMPORTANT:
    - We only have sensitivity in the lifetime range actually scanned.
      For τ outside [τ_min, τ_max] we treat the point as *not excluded*.
    """
    br_limits = np.asarray(br_limits, dtype=float)
    lifetimes = np.asarray(lifetimes, dtype=float)

    # Keep only positive, finite points
    valid = (
        np.isfinite(br_limits) & np.isfinite(lifetimes) &
        (br_limits > 0.0) & (lifetimes > 0.0)
    )
    if np.count_nonzero(valid) < 2:
        # Not enough information to build a meaningful curve
        return np.nan

    log_tau = np.log10(lifetimes[valid])
    log_br  = np.log10(br_limits[valid])

    # Sort by lifetime to make interp1d happy
    order = np.argsort(log_tau)
    log_tau_sorted = log_tau[order]
    log_br_sorted  = log_br[order]

    # Lifetime range where we *actually* have information
    tau_min = 10.0**log_tau_sorted[0]
    tau_max = 10.0**log_tau_sorted[-1]

    # Interpolator in log–log space; outside the range → NaN
    br_limit_interp = interp1d(
        log_tau_sorted,
        log_br_sorted,
        kind='linear',
        bounds_error=False,
    ...
    # Scan |U|^2 from 1e-12 to 1
    log_coupling_sq_range = np.linspace(-12.0, 0.0, 1000)
    coupling_sq_range = 10.0**log_coupling_sq_range

    excluded = []

    for coupling_sq in coupling_sq_range:
        # Expected BR and lifetime for this coupling
        br_expected  = br_w_to_hnl(coupling_sq, mass_gev, lepton)
        tau_expected = hnl_lifetime(coupling_sq, mass_gev)

        # If lifetime is unphysical or outside scanned range → no sensitivity
        if (not np.isfinite(tau_expected)) or (tau_expected <= 0.0):
            excluded.append(False)
            continue

        if (tau_expected < tau_min) or (tau_expected > tau_max):
            excluded.append(False)
            continue

        # Interpolate BR_limit at this lifetime
        log_tau_exp = np.log10(tau_expected)
        log_br_lim  = br_limit_interp(log_tau_exp)

        if not np.isfinite(log_br_lim):
            excluded.append(False)
            continue

        br_lim = 10.0**log_br_lim
    ...
    if np.any(excluded):
        # First excluded point = smallest |U|^2 with BR_expected > BR_limit
        first_idx = np.where(excluded)[0][0]
        coupling_sq_limit = coupling_sq_range[first_idx]
        return coupling_sq_limit

    # No exclusion in the scanned coupling range
    return np.nan
...
def main():
    ...
    if args.test:
        ...
        if len(masses_with_data) == 0:
            ...
            print("Example:")
            lepton_map = {'mu': 'muon', 'tau': 'tau', 'e': 'electron'}
            lepton_name = lepton_map.get(args.scenario, 'muon')
            print(f"  python decayProbPerEvent.py csv/HNL_mass_15.0_{lepton_name}.csv")
            return
