...
**Expected output:**
```
✓ Process directory created: /work/production/madgraph_production/work/hnl_muon_15.0GeV
...
| **Output** | `production/csv/simulation/` | `production/madgraph_production/csv/` |
| **Analysis** | `analysis_pbc/` | Same (compatible CSV format) |

**Both pipelines are needed** to cover the full 0.2-80 GeV mass range!

The analysis code (`analysis_pbc/limits/u2_limit_calculator.py`) can consume CSV files from **both** sources (they use the same format).
...
**Analysis Pipeline**:
- See `../../analysis_pbc/` for downstream analysis
